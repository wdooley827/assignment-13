...
//done in class
var where = function(arrOfObjects, critObj){
  var filteredListObjects = []

  for (var i = 0; i < arrOfObjects.length; i++){
    var singleObject = arrOfObjects[i]
    var objectMatch = true

    for (var propStr in critObj){
      if (critObj[propStr] !== singleObject[propStr]){
        objectMatch = false
      }
    }
    if (objectMatch){
      filteredListObjects.push(singleObject)
    }
  }
