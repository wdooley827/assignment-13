 ...
 *              }
**/
function showStats(inputNum,inputArray){

var newObj = {
  count_inputEqualTo: 0,
  count_inputGreaterThan: 0,
  count_inputLessThan: 0,
  setSize: inputArray.length,
}

  for (var i = 0; i < inputArray.length; i++){
    if (inputNum === inputArray[i]){
      newObj.count_inputEqualTo += 1
    }else if (inputNum > inputArray[i]){
      newObj.count_inputGreaterThan += 1
    }else if (inputNum < inputArray[i]){
      newObj.count_inputLessThan += 1
    }
  }
  return newObj
}
